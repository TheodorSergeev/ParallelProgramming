...
def plot_execution_time(filename):
    ...
    with open(filename, 'r') as csvfile:
        ...
        for row in plots:
            ...
            exec_time_seq.append(float(row[1]))
            exec_time_lin.append(float(row[2]))
            exec_time_cub.append(float(row[3]))
    ...
    ax1.set_title (r'Dependence of the execution time on the number of processes (merge sort)', fontsize=titlesize)
    ax1.plot(proc_num, exec_time_seq,  **params, label='sequential')
    ax1.plot(proc_num, exec_time_lin,  **params, label='parallel (linear)')
    ax1.plot(proc_num, exec_time_cub,  **params, label='parallel (hypercube)')
